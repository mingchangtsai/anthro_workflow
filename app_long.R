...
# Recent with timestamps via get_anthro (sorted by timestamp desc)
...
append_rows_batch <- function(rows, chunk_size = 200) {
  n <- length(rows)
  if (n == 0) return(TRUE)
  ok_all <- TRUE
  withProgress(message = 'Uploading...', value = 0, {
    for (i in seq(1, n, by = chunk_size)) {
      ...
      inc <- (j - i + 1) / max(1, n)
      ok <- append_rows(rows[i:j])
      ok_all <- ok_all && isTRUE(ok)
      incProgress(inc, detail = sprintf('Rows %d–%d of %d', i, j, n))
      if (!ok) break
    }
  })
  ok_all
...
server <- function(input, output, session) {
  ...
  output$recent_tbl <- renderDT({
    ...
    if (is.null(df) || nrow(df) == 0) {
      ...
      datatable(df2, options = list(dom = 't', paging = FALSE))
    }
  })
  ...
  build_row <- function(category, item, idx) {
    ...
    div(class="row-line measure-row",
      ...
      div(class="seg-sugg", div(tags$label("Suggest 3rd"), uiOutput(idsugg))),
  ...
  # Prefill using most recent date for athlete (recent_anthro -> get_anthro) with progress
  ...
  # ---- Batch upload: Excel -> Google Sheet (adds timestamp if missing) ----
  ...
  observeEvent(input$upload_btn, {
    ...
    ok <- append_rows_batch(rows, chunk_size = 200)
    if (ok) {
      ...
      output$upload_status <- renderText("Upload failed for one or more chunks.")
    }
  })

  # Submit & Save with duplicate confirmation
  observeEvent(input$submit, {
    ...
    # Duplicate check AFTER building rows
    ...
    ok <- append_rows(rows)
    if (ok) {
      output$status <- renderText("Saved to Google Sheet.")
      recent_data(fetch_recent_with_ts(10, progress = TRUE))
    } else {
  ...
  observeEvent(input$confirm_overwrite, {
    req(!is.null(pending_rows()))
    removeModal()
    rows <- pending_rows()
    athlete_key <- input$athlete
    date_key <- as.character(input$date)
    ok <- replace_rows_for_key(athlete_key, date_key, rows)
    if (ok) {
      output$status <- renderText("Existing record overwritten and saved to Google Sheet.")
      pending_rows(NULL)
      recent_data(fetch_recent_with_ts(10, progress = TRUE))
    } else {
