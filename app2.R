...
fetch_recent <- function(limit = 10) {
  ...
  if (!inherits(res, "try-error") && isTRUE(res$ok) && length(res$data) > 0) {
    ...
    if (!inherits(df, "try-error")) return(df %>% select(date, athlete))
  }
  tibble(date = character(), athlete = character())
}
...
ui <- fluidPage(
  tags$head(
    tags$style(HTML("
      ...
      /* Flex fractional layout for Section 2 rows */
      .measure-row { display: flex; align-items: flex-start; gap: 8px; }
      .seg-name { flex: 0 0 25%; }     /* ~3/12 */
      .seg-in { flex: 0 0 12.5%; }     /* ~1.5/12 each */
      .seg-sugg { flex: 0 0 25%; }     /* ~3/12 */
      .seg-val { flex: 0 0 12.5%; }    /* ~1.5/12 */
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      shinyTime::timeInput("time", "Time", value = strptime(format(Sys.time(), "%I:%M %p"), "%I:%M %p"),
                           seconds = FALSE),
      pickerInput("scale", "Scale", choices = scale_choices, multiple = FALSE,
                  options = pickerOptions(style = "btn-outline-primary")),
      hr(),
      actionButton("prefill_btn", "Prefill from most recent", class = "btn-outline-secondary", width = "100%"),
      br(), br(),
    ...
    mainPanel(
      ...
      div(class = "card",
          ...
          fluidRow(
            column(4, div(numericInput("usg","Urine Specific Gravity", value = NA, min = 0.950, max = 1.010, step = 0.001),
                          uiOutput("usg_warn"))),
...
server <- function(input, output, session) {
  ...
  output$usg_warn <- renderUI({
    ...
    if (v < 0.950 || v > 1.010) {
      span(class="warn","Warning: USG out of range (expected 0.950–1.010).")
    } else {
      span(class="muted","")
    }
  })
  ...
  # Recent list (sidebar)
  recent_data <- reactiveVal(fetch_recent(10))
  output$recent_tbl <- renderDT({
    df <- recent_data()
    if (is.null(df) || nrow(df) == 0) {
      datatable(data.frame(date = character(), athlete = character()), options = list(dom = 't', paging = FALSE))
    } else {
      datatable(df %>% select(date, athlete), options = list(dom = 't', paging = FALSE))
    }
  })
  ...
  build_row <- function(category, item, idx) {
    ...
    div(class="row-line measure-row",
      ...
      div(class="seg-sugg", div(tags$label("3rd Measurement?"), uiOutput(idsugg))),
  ...
  # Prefill using most recent date for athlete (recent_anthro -> get_anthro)
  ...
  observeEvent(input$prefill_btn, {
    ...
    showNotification(glue("Prefill using date: {latest_date}"), type = "message", duration = 5)
    ...
    if (inherits(got, "try-error") || !isTRUE(got$ok) || length(got$data) == 0) {
      showNotification("No rows from get_anthro.", type = "warning"); return(invisible(NULL))
    }
    ...
    # ---- Prefill Section 1 ----
    first <- dat[1,]
    if ("practitioner" %in% names(first)) updateSelectizeInput(session, "practitioner", selected = first$practitioner)
    updateSelectizeInput(session, "athlete", selected = input$athlete)

    if ("date" %in% names(first)) {
      d_try <- suppressWarnings(lubridate::ymd(first$date))
      if (is.na(d_try)) d_try <- suppressWarnings(lubridate::ymd_hms(first$date))
      if (!is.na(d_try)) updateDateInput(session, "date", value = as.Date(d_try))
    }
    if ("time" %in% names(first)) {
      t_try <- suppressWarnings(lubridate::ymd_hms(first$time))
      if (!is.na(t_try)) shinyTime::updateTimeInput(session, "time", value = strptime(format(t_try, "%I:%M %p"), "%I:%M %p"))
    }
    if ("scale" %in% names(first))    updatePickerInput(session, "scale", selected = first$scale)
    if ("fasted" %in% names(first))   updatePrettyRadioButtons(session, "fasted", selected = first$fasted)
    if ("day_of_cycle" %in% names(first)) updatePickerInput(session, "doc", selected = as.character(first$day_of_cycle))
    if ("birth_control" %in% names(first)) updatePrettyRadioButtons(session, "bc", selected = first$birth_control)
    if ("creatine" %in% names(first)) updatePrettyRadioButtons(session, "creatine", selected = first$creatine)
    if ("usg" %in% names(first))      updateNumericInput(session, "usg", value = suppressWarnings(as.numeric(first$usg)))
    if ("caliper" %in% names(first))  updatePickerInput(session, "caliper", selected = first$caliper)
    if ("comments" %in% names(first)) updateTextAreaInput(session, "comments", value = first$comments %||% "")
    ...
    for (cat in names(measures_list)) {
      ...
      for (i in seq_along(items)) {
        ...
        row <- dat %>% filter(.data$Category == cat, .data$Measure == meas) %>% slice(1)
        if (nrow(row) == 0) next

        v1 <- suppressWarnings(as.numeric(row$m1)); if (is.na(v1) && row$m1 == "") v1 <- NA_real_
        v2 <- suppressWarnings(as.numeric(row$m2)); if (is.na(v2) && row$m2 == "") v2 <- NA_real_
        v3 <- suppressWarnings(as.numeric(row$m3)); if (is.na(v3) && row$m3 == "") v3 <- NA_real_

        # Respect bounds: if value out of range, set NA so user must re-enter
    ...
    showNotification(glue("Prefilled {nrow(dat)} rows for {input$athlete} ({latest_date})."),
  ...
  observeEvent(input$submit, {
    ...
    # Build rows for each measure
    rows <- list()
    ts_now <- as.character(Sys.time())

    # derive session/time fields
    ...
    for (cat in names(measures_list)) {
      ...
      for (i in seq_along(items)) {
        ...
        # bounds enforcement: do not accept out-of-range
        bnds <- bounds_for(cat, meas); lo <- bnds[1]; hi <- bnds[2]
        ...
        rows[[length(rows)+1]] <- list(
          ...
          Category      = cat,
          Measure       = meas,
          m1            = ifelse(is.na(v1), "", v1),
          m2            = ifelse(is.na(v2), "", v2),
          m3            = ifelse(is.na(v3), "", v3),
          suggest       = s_msg %||% "",
          value         = ifelse(is.na(val), "", round(val, 2))
        )
      }
    }
    ...
    if (ok) {
      ...
      recent_data(fetch_recent(10))
    } else {
      output$status <- renderText("Error: could not save to datavase.")
    }
  })
}
