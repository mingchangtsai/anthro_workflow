  ...
  navbarPage(
    ...
    tabPanel(
      ...
      fluidPage(
        ...
        br(),
        div(style="width:70%;", DTOutput("report_tbl")),  # 70% width table
        tags$script(HTML("
...
server <- function(input, output, session) {
  ...
  fetch_dates_for_athlete <- function(ath) {
    ...
    if (!inherits(res, "try-error") && isTRUE(res$ok) && length(res$data) > 0) {
      d <- try(as_tibble(jsonlite::fromJSON(jsonlite::toJSON(res$data))), silent = TRUE)
      if (!inherits(d, "try-error") && "norm" %in% names(d)) {
        return(unique(as.character(d$norm)))
      }
    }
    rec <- fetch_recent(2000)
    rec %>%
      filter(tolower(trimws(athlete)) == tolower(trimws(ath))) %>%
      pull(date) %>%
      as.character() %>%
      unique()
  }
  ...
  fetch_athlete_history <- function(ath) {
    ...
    for (i in seq_along(dates)) {
      got <- try(api_get(list(action = "get_anthro", athlete = ath, date = dates[[i]], cb = as.integer(Sys.time()))), silent = TRUE)
      if (!inherits(got, "try-error") && isTRUE(got$ok) && length(got$data) > 0) {
        dat <- try(tibble::as_tibble(jsonlite::fromJSON(jsonlite::toJSON(got$data), flatten = TRUE)), silent = TRUE)
        if (!inherits(dat, "try-error") && nrow(dat) > 0) rows[[i]] <- dat[1,]
      }
    }
      ...
      withProgress(message = "Rendering reports…", value = 0, {
        ...
        for (k in seq_along(sel_idx)) {
          ...
          renv$AthleteData <- hist %>%
            dplyr::rename(AthleteName = athlete) %>%
            dplyr::mutate(Date = suppressWarnings(as.Date(date %||% Date)))
